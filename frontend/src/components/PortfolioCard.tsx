...
export function PortfolioCard({ exchanges, totalUnrealizedPnl }: PortfolioCardProps) {
  ...
  return (
    <div className="glass-card rounded-2xl p-6 border border-white/5 shadow-xl relative overflow-hidden group hover:border-white/10 transition-colors">
      ...
      <h3 className="text-lg font-semibold mb-6 text-white tracking-wide">Net PnL by Exchange</h3>
      ...
      <div className="space-y-4 relative z-10">
        {displayExchanges.map((ex: ExchangeData, idx: number) => {
          ...
          return (
            <div key={idx} className="flex justify-between items-center bg-white/[0.04] p-4 rounded-xl border border-white/10 backdrop-blur-md hover:bg-white/[0.08] transition-colors">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-full p-[1px] shadow-[0_0_15px_rgba(255,255,255,0.1)] ${isBinance ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' : isOkx ? 'bg-gradient-to-br from-blue-400 to-blue-600' : 'bg-gradient-to-br from-gray-400 to-gray-600'}`}>
                  <div className={`w-full h-full bg-black/60 rounded-full flex items-center justify-center font-bold text-sm ${isBinance ? 'text-yellow-400' : isOkx ? 'text-blue-400' : 'text-gray-400'}`}>
                    {symbol}
                  </div>
                </div>
                <div>
                  <span className="font-semibold text-white tracking-wide block capitalize">{ex.exchange_id}</span>
                  <span className="text-[10px] text-gray-400 uppercase tracking-widest">{ex.trade_count ? `${ex.trade_count} Trades` : 'Account'}</span>
                </div>
              </div>
      ...
      <div className="mt-8 pt-6 border-t border-white/10 relative z-10">
        <div className="flex justify-between items-end text-sm">
          <span className="text-gray-400 uppercase tracking-widest text-xs font-semibold">Total Realized PnL</span>
